...
void cut_region()
{
  ...
  for(i = NumPartNew = 0; i < NumPartCut; i++)
    if(dabs(PV.X[i]) < All.CutSizeHalf && dabs(PV.Y[i]) < All.CutSizeHalf && dabs(PV.Z[i]) < All.CutSizeHalf)
      {
	PartFlag[i] = 1;
  ...
  sprintf(snapdir, "%s/%s", All.Path, All.CutBase);

  mkdir(snapdir, 0755);

  if(All.CutCosm)
    sprintf(All.SnapFile, "%s/%s/%s_ic", All.Path, All.CutBase, All.CutBase);
  else
    sprintf(All.SnapFile, "%s/%s/%s_000", All.Path, All.CutBase, All.CutBase);
  ...
  if(BlockFlag[block])
      ...
      if(block == IO_POS)
	{
	  ...
	  for(i = j = 0; i < NumPartCut; i++)
	    if(PartFlag[i])
	      {
		PV.X[j] = PV.X[i] + All.CutSizeHalf;
		PV.Y[j] = PV.Y[i] + All.CutSizeHalf;
		PV.Z[j] = PV.Z[i] + All.CutSizeHalf;

		if(PV.X[j] < 0 || PV.Y[j] < 0 || PV.Z[j] < 0)
		  terminate("Problem: particle position < 0! %d %g %g %g", i, PV.X[j], PV.Y[j], PV.Z[j]);

		if(PV.X[j] >= All.BoxSize || PV.Y[j] >= All.BoxSize || PV.Z[j] >= All.BoxSize)
		  terminate("Problem: particle position >= box size! %d %g %g %g", i, PV.X[j], PV.Y[j], PV.Z[j]);
