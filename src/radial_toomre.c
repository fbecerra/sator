...
void radial()
{
  ...
  int block_geff, block_rates, block_cool, block_collapse, block_sidm_density, block_toomre;
  ...
  double *loc_vel_rad, *vel_rad, *vel_rot, *loc_vel_turb, *vel_turb, *kep_ratio;
  ...
  double vrad, vradx, vrady, vradz, vrotx, vroty, vrotz, vturbx, vturby, vturbz, vkep;
  ...
  double *loc_mu_rad, *mu_rad, *loc_geff, *geff, *loc_escfrac, *escfrac;
  ...
  double cool_rate, esc_frac, heat_rate, abhi, tau_CIE, kdh;
  FILE *file;
...
*/
  ...
  for(i = 0; i < NumGas; i++)
      ...
      if(block_cool)
	{
	  ...
	  cool_rate = get_H2_cool_rate(i);

	  esc_frac = get_H2_esc_frac(i);

	  loc_escfrac[j] += PV.Mass[i] * log10(esc_frac);

	  cool_rate *= esc_frac;

	  tau_CIE = dmax(pow(PV.NH[i] / 1.4e16, 2.8), 1e-5);

	  //cool_rate += 2.289e-49 * pow(PV.Temp[i], 4) * PV.NH[i] * (2 * PV.AbH2[i] * PV.NH[i]) * dmin(1 / ((1 + tau_CIE) * (1 + tau_CIE / 10)), 1);

	  kdh = 5.24e-7 * pow(PV.Temp[i], -0.485) * exp(-5.2e4 / PV.Temp[i]);

	  abhi = dmax(1 - 2 * PV.AbH2[i] - PV.AbHII[i], 0);

	  //cool_rate += 4.48 * ELECTRON_VOLT * kdh * abhi * PV.AbH2[i] * PV.NH[i] * PV.NH[i];

	  tcool = PV.U[i] * PV.Rho[i] / cool_rate;

	  loc_h2rate[j] += PV.Mass[i] * log10(tcool / tff);

	  heat_rate = get_3b_form_heat(i);

	  theat = PV.U[i] * PV.Rho[i] / heat_rate;

	  loc_chemrate[j] += PV.Mass[i] * log10(theat / tff);

	  tcool = PV.U[i] * PV.Rho[i] / (cool_rate - heat_rate);
...
//      sigma_mass[i] = enc[i] * SOLAR_MASS / M_PI / (fac * rad[i]) / (fac * rad[i]);
  ...
  if(BlockFlag[IO_VEL])
      ...
      for(i = 0; i < All.RadBins; i++)
	{
	  ...
	  vkep = sqrt(GRAVITY * enc[i] * SOLAR_MASS / (fac * rad[i])) / 1e5;

	  kep_ratio[i] = vel_rot[i] / vkep;
	}
